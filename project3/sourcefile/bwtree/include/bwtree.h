...
class BwTree : public BwTreeBase {
 ...
 public:
  ...
  NO_ASAN void InitMappingTable() {
    ...
    mapping_table_cnt = (std::atomic<int> *)mmap(nullptr, sizeof(int) * MAPPING_TABLE_SIZE,
                                                              PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
...
// they could call CAS and change the key
  ...
  NO_ASAN inline const BaseNode *GetNode(const NodeID node_id) {
    ...
    mapping_table_cnt[node_id].fetch_add(1);
  ...
  retry_traverse:
    ...
    while (1) {
      ...
      INDEX_LOG_TRACE("After LoadNodeID");
      if (context_p->abort_flag) {
   ...
   * If both are nullptr then we just traverse and do not do anything
   */
  NO_ASAN const KeyValuePair *Traverse_LRU(Context *context_p, const ValueType *value_p, std::pair<int, bool> *index_pair_p,
                                       bool unique_key = false) {
    // For value collection it always returns nullptr
...
// NOISEPAGE_ASSERT(it == it2);
  ...
  NO_ASAN LeafNode *CollectAllValuesOnLeaf(NodeSnapshot *snapshot_p, LeafNode *leaf_node_p = nullptr) {
    ...
    INDEX_LOG_TRACE("leaf_node_p->GetSize(): %d, node_p->GetItemCount(): %d", leaf_node_p->GetSize(), node_p->GetItemCount());
...
#endif
    INDEX_LOG_TRACE("parent NodeID %d", GetLatestParentNodeSnapshot(context_p)->node_id);
    INDEX_LOG_TRACE("parent Nodep %p", GetLatestParentNodeSnapshot(context_p)->node_p);
    INDEX_LOG_TRACE("child NodeID %d", GetLatestNodeSnapshot(context_p)->node_id);
    INDEX_LOG_TRACE("child Nodep %p", GetLatestNodeSnapshot(context_p)->node_p);
  ...
  NO_ASAN void LoadNodeID(NodeID node_id, Context *context_p) {
    ...
    INDEX_LOG_TRACE("After FinishPartialSMO");
    if (context_p->abort_flag) {
      return;
    }
    ...
    INDEX_LOG_TRACE("After TryConsolidateNode");
    AdjustNodeSize(context_p);
    INDEX_LOG_TRACE("After AdjustNodeSize");
  }
  ...
  NO_ASAN bool Insert(const KeyType &key, const ValueType &value, bool unique_key = false) {
    ...
    while (1) {
#ifdef BWTREE_DEBUG
    insert_trial_count.fetch_add(1);
#endif
      Context context{key};
      std::pair<int, bool> index_pair;
      ...
      if (ret) {
        ...
        // if (max_level < context.current_level) {
        //   max_level = context.current_level;
        // }
  ...
  NO_ASAN bool Insert_LRU(const KeyType &key, const ValueType &value, bool unique_key = false) {
    INDEX_LOG_TRACE("Insert_LRU called");
    ...
    insert_trial_count.fetch_add(1);
      ...
      // then return the position of the node using next_key_p
      // if there is none then return nullptr
      const KeyValuePair *item_p = Traverse_LRU(&context, &value, &index_pair, unique_key);
 ...
 public:
  ...
  std::atomic<int> *mapping_table_cnt;
  ...
  std::atomic<uint64_t> insert_trial_count;

...
struct BwTreeTestUtil {
  ...
  using TreeType =
      bwtree::BwTree<int64_t, int64_t, BwTreeTestUtil::KeyComparator, BwTreeTestUtil::KeyEqualityChecker>;
  ...
  static TreeType::LRUcache *GetLRUcache(int cache_size) {
    auto *cache_ptr = new TreeType::LRUcache(cache_size);
    return cache_ptr;
  }
};
